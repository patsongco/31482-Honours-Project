...
from model.KGAT_hake import KGAT_hake
from utility.parser_kgat_hake import *
...
def train(args):
    ...
    model = KGAT_hake(args, data.n_users, data.n_entities, data.n_relations, user_pre_embed, item_pre_embed)
...
def predict(args):
    ...
    # load model
    model = KGAT_hake(args, data.n_users, data.n_entities, data.n_relations)
    model = load_model(model, args.pretrain_model_path)
    model.to(device)
    # if n_gpu > 1:
    #     model = nn.parallel.DistributedDataParallel(model)
