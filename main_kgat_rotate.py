...
from model.KGAT_rotate import KGAT_rotatE
from utility.parser_kgat_rotate import *
...
def train(args):
    ...
    model = KGAT_rotatE(args, data.n_users, data.n_entities, data.n_relations, user_pre_embed, item_pre_embed)
...
def predict(args):
    ...
    # load model
    model = KGAT_rotatE(args, data.n_users, data.n_entities, data.n_relations)
    model = load_model(model, args.pretrain_model_path)
    model.to(device)
    # if n_gpu > 1:
    #     model = nn.parallel.DistributedDataParallel(model)
