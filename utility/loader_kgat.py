...
class DataLoaderKGAT(object):
    ...
    def __init__(self, args, logging):
        ...
        train_file = os.path.join(data_dir, 'new_train.txt')
        test_file = os.path.join(data_dir, 'new_test.txt')
        kg_file = os.path.join(data_dir, "new_kg_final.txt")
    ...
    def generate_cf_batch(self, user_dict):
        ...
        if self.cf_batch_size <= len(exist_users):
            batch_user = random.sample(list(exist_users), self.cf_batch_size)
        else:
            batch_user = [random.choice(list(exist_users)) for _ in range(self.cf_batch_size)]
